...
for i = 1:length(EsN0_fine)
    selected = 0;
    for s = nMod:-1:1   % highest order first
...
title('BER vs SNR with AM', 'FontSize', 14);
...
ylabel('BER', 'FontSize', 13);
title(sprintf('BER vs Distance, h_{UAV}=%dm', h_uav), ...
    'FontSize', 14);
...
xlabel('Horizontal Distance R (m)', 'FontSize', 13);
ylabel('BER', 'FontSize', 13);
title(sprintf('BER vs Distance with AM, h_{UAV}=%dm', h_uav), ...
    'FontSize', 14);
legend('Location', 'southeast', 'FontSize', 10);
xlim([0 R_fine(end)]);
ylim(ylims);
